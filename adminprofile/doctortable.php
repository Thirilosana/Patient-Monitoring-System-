...
<body>
   ...
   <div id='cssmenu'>
      <ul>
         ...
         <li class='last'>
            <a href="adminlogin.php"><span>Sign Out</span></a>
         </li>
      </ul>
   </div>
  ...
  -->
    <table id="fresh-table" class="table">
      ...
      <?php
           ...
           $sql = "SELECT fname, mobileno, dob, gender, speciality, place FROM doctor_reg";
           $result = $con-> query($sql);

           if($result-> num_rows > 0){
           while($row = $result-> fetch_assoc()){

            echo "<tr><td>".$row["fname"]."</td><td>".$row["mobileno"]."</td><td>".$row["dob"]."</td><td>".$row["gender"]."</td><td>".$row["speciality"]."</td><td>".$row["place"]."</td></tr>";
           }
           echo "</table>";
           }
           else{
